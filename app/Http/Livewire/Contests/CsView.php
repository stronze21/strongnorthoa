...
class CsView extends Component
{
    ...
    public function render()
    {
        ...
        $select = "SELECT lifechanger, cs.team_builder,COUNT(cs.cs_id) as 'shows', main.id as maid, cs.contest_id,
        ...
        $order_by = " ORDER BY sales DESC, sets DESC, shows DESC;";
        $join = " join order_agreements as main on cs.cs_id = main.cs_id";
        $where2 = "";

        if ($contest->for_team_builders) {
            $group_by = " GROUP BY cs.team_builder";
        } else {
            $group_by = " GROUP BY cs.user_id";
        }
        ...
        $query = $select . $join . $where . $where2 . $group_by . $order_by;
