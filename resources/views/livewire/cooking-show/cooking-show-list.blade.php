<div>
    <div class="flex flex-col items-start justify-between mb-6 md:flex-row md:items-center">
        <h1 class="text-2xl font-bold">Cooking Shows</h1>
        <a href="{{ route('cooking-shows.create') }}" class="btn btn-primary">
                        ...
                        @forelse ($cookingShows as $show)
                            <tr class="hover">
                                <td>
                                    ...
                                    @if ($show->user)
                                        <div class="text-sm opacity-50">By: {{ $show->user->fullname() }}</div>
                                    @endif
                                </td>
                                <td>
                                ...
                                <td>
                                    <div class="flex space-x-1">
                                        <a href="{{ route('cooking-shows.details', $show->cs_id) }}"
                                        ...
                                        <a href="{{ route('cooking-shows.edit', $show->cs_id) }}"
                                            class="btn btn-square btn-ghost btn-xs">
                                            <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5" fill="none"
                                                viewBox="0 0 24 24" stroke="currentColor">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                        ...
                                        <button wire:click="delete({{ $show->cs_id }})"
                                            class="btn btn-square btn-ghost btn-xs"
                                            onclick="confirm('Are you sure you want to delete this cooking show?') || event.stopImmediatePropagation()">
                                            <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5" fill="none"
                                                viewBox="0 0 24 24" stroke="currentColor">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
