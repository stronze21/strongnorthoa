    ...
    <div class="mx-auto mb-5 max-w-7xl sm:px-6 lg:px-8">
        <div class="flex justify-between mb-2 overflow-hidden border-b sm:rounded-lg">
            <div class="p-2border-gray-200">
                    ...
                    <label class="my-2 btn btn-sm btn-secondary ms-2" for="update_details">Edit Additional Details</label>
                    <a class="my-2 btn btn-sm btn-info ms-2" href="{{route('signaturepad', $oa->id)}}">Signature</a>
            </div>
            <div>
                ...
                <div class="p-6">
                    ...
                    <div class="mt-2">
                        <table class="table w-full border table-compact">
                            ...
                            <tbody>
                                @forelse ($oa->items()->get() as $order)
                                    <tr class="{!! $order->remarks == 'Composed of:' ? 'active"' : '' !!} cursor-pointer hover" onclick="delete_item('{{$order->id}}', 'item')">
                                        <td>{!! $order->product->tblset_id ? '<span class="font-bold">'.$order->product->product_description.'</span> Composed of:'  : $order->product->product_description !!}</td>
                        ...
                        <table class="table w-full border table-compact">
                            ...
                            <tbody>
                                @forelse ($oa->gifts()->get() as $order)
                                    <tr class="cursor-pointer hover" onclick="delete_item('{{$order->id}}', 'gift')">
                                        <td>{{$order->type}}</td>
                            ...
                            <tfoot>
                                ...
                                <tr class='table-light'><td class="text-right" colspan='4'><strong>TOTAL:</strong></td> <td class="text-right" colspan="2"><strong>&#8369; {{number_format($total,2)}}</strong></td></tr>
                            </tfoot>
                        </table>
                        <div class="grid grid-cols-2 gap-1 px-3 py-5 border">
                            ...
                            <div class="flex flex-col">
                                <div class="flex">
                                    <span class=" text-ellipsis">Current Spirit of Success Level: </span> <span>{{$oa->current_level}}</span>
                                </div>
                                <div class="flex">
                                    <span>Initial Investment: </span> <span class="ml-1">{{number_format($oa->initial_investment, 2)}}</span>
                                </div>
                                <div class="flex">
                                    <span>Balance: </span> <span class="ml-1">{{number_format($total - $oa->initial_investment,2)}}</span>
                                </div>
                                <div class="flex">
                                    <span>Terms: </span> <span class="ml-1">{{$oa->terms}}</span>
                                </div>
                                <p class="text-ellipsis">
                                    <small>Checks payable only to <span class="font-bold uppercase">StrongNorth Cookware Trading</span></small>
                                </p>
            ...
            <div class="w-full py-4">
                ...
                <div class="w-full mb-2 form-control">
                    <label class="input-group">
                      <span>Qty</span>
                      <input type="number" min="1" class="w-full input input-bordered" wire:model="item_qty"/>
                    </label>
                    @error('item_qty')
        ...
        <div class="modal-box">
            ...
            <div class="w-full py-4">
                ...
                <div class="w-full mb-2 form-control">
                    <label class="input-group">
                      <span>Qty</span>
                      <input type="number" min="1" class="w-full input input-bordered" wire:model="gift_qty"/>
                    </label>
                </div>
                <div class="w-full mb-2 form-control">
                    <label class="input-group">
            ...
            <div class="w-full p-4">
                ...
                <div class="w-full mb-2 form-control">
                    <label class="input-group">
                      ...
                      <input type="date" min="{{date('Y-m-d')}}" class="w-full input input-bordered" wire:model="delivery_date"/>
                    </label>
                </div>
                <div class="w-full mb-2 form-control">
                    <label class="input-group">
                ...
                <div class="w-full pr-3 mb-2 form-control">
                    <label class="input-group">
                      ...
                      <input type="number" step="0.01" class="w-full input input-bordered" wire:model="initial_investment"/>
                    </label>
                </div>
                <div class="w-full mb-2 form-control">
                    <label class="input-group">
